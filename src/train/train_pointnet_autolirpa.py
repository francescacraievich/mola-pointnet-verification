...
def load_data_with_nsga3_labels(data_dir: Path, nsga3_dir: Path, run_id: int = 10):
    ...
    print(f"NSGA-III weights: linearity={weights['linearity']:.4f}, "
    ...
    print(f"Train: {len(train_xyz)} samples, "
          f"CRITICAL: {(train_labels==0).sum()}, NON_CRITICAL: {(train_labels==1).sum()}")
    print(f"Test: {len(test_xyz)} samples, "
...
def main():
    ...
    parser.add_argument('--nsga3-dir', type=str, default='../mola-adversarial-nsga3/src/results/runs')
    ...
    train_dataset = TensorDataset(
        torch.from_numpy(train_xyz),
        torch.from_numpy(train_labels).long()
    )
    test_dataset = TensorDataset(
        torch.from_numpy(test_xyz),
