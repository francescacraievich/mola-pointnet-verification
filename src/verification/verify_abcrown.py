...
# Global state for saving on interrupt
_save_state = {
    "sample_details": [],
    "summary": {},
    "args": None,
...
def save_partial_results(reason="interrupted"):
    """Save current results to file."""
    if not _save_state["sample_details"]:
        print("\nNo results to save.")
        return
    ...
    elapsed = time.time() - _save_state["start_time"] if _save_state["start_time"] else 0

    # Build summary data
    summary_data = {}
    for eps in _save_state["args"].epsilon:
        s = _save_state["summary"].get(eps, {"verified": 0, "unsafe": 0, "timeout": 0})
    ...
    final_results = {
        "metadata": {
            ...
            "model_accuracy": _save_state["test_accuracy"],
            "config": _save_state["args"].config,
            "timestamp": datetime.now().isoformat(),
            "total_time_seconds": round(elapsed, 2),
            "timeout_per_sample": _save_state["args"].timeout,
            "status": reason,
            "samples_completed": len(_save_state["sample_details"]),
            "samples_total": len(_save_state["selected_samples"]),
        },
        "parameters": {
            "samples": _save_state["selected_samples"],
            "epsilons": _save_state["args"].epsilon,
            "selection_method": (
                _save_state["args"].by_margin
                if _save_state["args"].by_margin
                else ("manual" if _save_state["args"].samples else "random")
            ),
        },
        "summary": summary_data,
        "details": _save_state["sample_details"],
    }
...
def signal_handler(sig, frame):
    """Handle Ctrl+C by saving results before exit."""
    print("\n\nInterrupted by user (Ctrl+C). Saving results...")
    save_partial_results("interrupted_by_user")
    sys.exit(0)
...
def load_model():
    ...
    checkpoint = torch.load(
        BASE_DIR / "saved_models/pointnet_autolirpa_512.pth", map_location="cpu", weights_only=True
    )
...
def select_samples(args, sample_info):
    ...
    indices = np.random.choice(
        len(correct_samples), min(args.n_samples, len(correct_samples)), replace=False
    )
...
def main():
    ...
    header = f"{'Sample':>6} | {'Margin':>7} | {'Label':>8} | " + " | ".join(
        f"{h:>10}" for h in eps_headers
    )
    print(header)
    print("-" * len(header))
    ...
    _save_state["sample_details"] = sample_details
    _save_state["summary"] = summary
    _save_state["args"] = args
    _save_state["test_accuracy"] = test_accuracy
    _save_state["selected_samples"] = selected_samples
    _save_state["start_time"] = total_start_time
    ...
    for eps in args.epsilon:
        ...
        rate = s["verified"] / total * 100 if total > 0 else 0
        print(
            f"{eps:>10.3f} | {s['verified']:>10} | {s['unsafe']:>10} | {s['timeout']:>10} | {rate:>9.1f}%"
        )
        summary_data[str(eps)] = {
            "epsilon": eps,
            "verified": s["verified"],
    ...
    final_results = {
        ...
        "parameters": {
            ...
            "selection_method": (
                args.by_margin if args.by_margin else ("manual" if args.samples else "random")
            ),
        },
        "summary": summary_data,
    ...
    with open(md_path, "w") as f:
        ...
        f.write("\n## Details\n\n")
        f.write(
            "| Sample | Margin | Label | " + " | ".join([f"ε={e}" for e in args.epsilon]) + " |\n"
        )
